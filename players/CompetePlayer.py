"""
MiniMax Player with AlphaBeta pruning and global time BEST PLAYER EVER!
"""
...
class Player(AbstractPlayer):
    def __init__(self, game_time, penalty_score):
        ...
        self.turn_count = 0
        self.fruits = []
        self.solver = AlphaBeta(self.utility, self.succ, self.perform_move)
        ...
        self.turn_limit = None      # function calculating the time limit for the current turn

        # heuristic weights
        self.weights = utils.get_weights(simple=0.3, rival_dist=0.3, closest_fruit=0.1,
                                         score_diff=0.2, fruits_taken=0.1)
    ...
    def set_game_params(self, board):
        ...
        for x, y in zip(coordinates[0], coordinates[1]):
            fruit = (x, y)
            min_fruit = min(self.board[fruit], min_fruit)
            fruits_counter += 1
            self.fruits.append(fruit)

        if self.penalty_score < int(min_fruit * 0.3 * fruits_counter) + 1:
            self.weights = utils.get_weights(simple=0.2, rival_dist=0, closest_fruit=0.2, score_diff=0.4,
                                             fruits_taken=0.3)
    ...
    def make_move(self, time_limit, players_score):
        ...
        self.TIME_THRESHOLD = 0.05 * self.turn_limit()

        # create state object for the AlphaBeta algorithm
        ...
        self.time_left = lambda: self.turn_limit() - (time.time() - start)
        ...
        try:
            ...
            depth = 1
            while True:
                alphabeta, direction = self.solver.search(start_state, depth, True)
                depth += 1
        finally:
            self.perform_move(direction)                        # update the board
            self.elapsed_time = (time.time() - self.game_start)   # update leftover game time
            ##self.set_efficient_turn_limit()  # update again the time limit for the next turn
            return direction
    ...
    def update_fruits(self, fruits_on_board_dict):
        ...
        if not fruits_on_board_dict.keys():
            self.weights = utils.get_weights(simple=0.4, rival_dist=0.4, closest_fruit=0, score_diff=0.2)
    ...
    def set_efficient_turn_limit(self):
        self.turn_limit = self.turn_limit / 2

    def set_turn_limit(self):
        free_blocks = len(np.where(self.board == 0)[0]) + len(np.where(self.board > 2)[0])
        free_blocks = int(free_blocks / 2)
        self.turn_limit = lambda: self.game_time / free_blocks

    def set_turn_limit_2(self):
        my_game_time = self.game_time / 2
        fruits_turns = min(len(self.board[0]), len(self.board))

        num_avail_slots = len(np.where(self.board == 0)[0]) + len(np.where(self.board > 2)[0])
        num_leftover_turns = int(num_avail_slots / 2) - fruits_turns + 1

        weight = 0.7
        turn_limit_with_fruits = (my_game_time / fruits_turns) * weight
        turn_limit_without_fruits = (my_game_time / num_leftover_turns) * (1-weight)

        self.turn_limit = lambda: turn_limit_without_fruits if not self.fruits else turn_limit_with_fruits
